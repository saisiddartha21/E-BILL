...
export default function EWayBillPage() {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/eway-bill?status=${statusFilter}&search=${search}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg flex items-start text-blue-800">
        ...
        <div>
          <p className="font-medium">E-Way Bill integration requires authorized GSP credentials.</p>
          <p className="text-sm mt-1 text-blue-700">Configure your API credentials in environment variables to enable automated submission.</p>
        </div>
      </div>
              ...
              {loading ? (
                ...
                invoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-center">{getStatusBadge(invoice.eWayBillStatus)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{invoice.ewayBillNo || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link href={`/e-way-bill/${invoice.id}`} className="text-blue-600 hover:text-blue-900 inline-flex items-center">
