...
export default function PurchaseReturnsPage() {
  ...
  const fetchReturns = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/purchase-returns?search=${search}`);
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Link href="/purchase-returns/new">
          <Button><Plus className="w-4 h-4 mr-2" /> New Purchase Return</Button>
        </Link>
      </div>
      ...
      <div className="flex items-center space-x-4">
        <div className="relative w-72">
          ...
          <Input 
            placeholder="Search by PR# or Purchase..." 
            value={search}
            ...
            {loading ? (
              ...
              returns.map((r: any) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>{r.purchase?.purchaseNumber}</TableCell>
